...
const perspectiveTransform = async (command) => {
  ...
  if (!layer) {
    throw new Error(
      `perspectiveTransform : Could not find layerId : ${layerId}`,
    );
  }
    ...
    let commands = [
      {
        ...
        rectangle: {
          ...
          bottom: {
            _unit: "pixelsUnit",
            _value: bounds.top + (bl.y / 100) * h,
          },
          right: {
            _unit: "pixelsUnit",
            _value: bounds.left + (tr.x / 100) * w,
          },
        },
        interfaceIconFrameDimmed: {
          _enum: "interpolationType",
...
const warpTransform = async (command) => {
  ...
  let horizontalDistortion =
    options.horizontalDistortion !== undefined
      ? options.horizontalDistortion
      : 0;
  let verticalDistortion =
    options.verticalDistortion !== undefined ? options.verticalDistortion : 0;
    ...
    let commands = [
      {
        ...
        warp: {
          ...
          warpValue: bend,
          warpPerspective: horizontalDistortion,
          warpPerspectiveOther: verticalDistortion,
          warpRotate: {
            _enum: "orientation",
            _value: "horizontal",
          },
